...
const els = {
  ...
  hotSupplyLabel: document.getElementById("hotSupplyLabel"),
  coldSupplyLabel: document.getElementById("coldSupplyLabel"),
};
...
function tempToColor(tempC) {
  // 10°C -> синий (холодная), 90°C+ -> янтарно-красный (горячая)
...
function applyState(state) {
  ...
  // --- подписи температур магистралей на мнемосхеме ---
  els.hotSupplyLabel.textContent = `HOT SUPPLY ${Math.round(state.input_temp_hot)}°C`;
  els.coldSupplyLabel.textContent = `COLD SUPPLY ${Math.round(state.input_temp_cold)}°C`;
  ...
  //     клапанами притока и нагревателем (после травел-тайм динамики и
  //     блокировок); могут отличаться от уставки оператора (slider) ---
  ...
  if (!userIsDragging) {
    for (const { input, out, tag, unit } of sliders) {
      if (document.activeElement !== input) {
        ...
        input.value = value;
        out.textContent = `${Math.round(value * 10) / 10}${unit}`;
      }
    }
  }
}
