...
nodes = {
    ...
    "SetpointTemperature": boiler.get_child("2:SetpointTemperature"),
    "SetpointLevel": boiler.get_child("2:SetpointLevel"),
...
def get_data():
    try:
        return jsonify({
            ...
            "setpoint_temp": round(nodes["SetpointTemperature"].get_value(), 1),
            "setpoint_level": round(nodes["SetpointLevel"].get_value(), 1),
            "auto_mode": bool(nodes["AutoMode"].get_value()),
        })
...
def set_setpoint():
    try:
        ...
        if name == "SetpointTemperature":
            nodes["SetpointTemperature"].set_value(value)
            return jsonify({"status": "ok"})
        elif name == "SetpointLevel":
            nodes["SetpointLevel"].set_value(value)
            return jsonify({"status": "ok"})
        else:
            return jsonify({"error": "Invalid setpoint name"}), 400
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def set_auto_mode():
    try:
        mode = bool(request.json.get("mode"))
        nodes["AutoMode"].set_value(mode)
        return jsonify({"status": "ok"})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
